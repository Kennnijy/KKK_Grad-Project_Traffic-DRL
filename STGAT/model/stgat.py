...
class STGATBlock(nn.Module):
    def __init__(self, cuda, in_channels, spatial_channels, out_channels,
        ...
        self.attentions = nn.ModuleList([GraphAttentionLayer(out_channels*(num_timesteps_input), spatial_channels, num_nodes=num_nodes,
                            dropout=dropout, alpha=alpha, concat=True) for _ in range(nheads)])
        
        self.relu = nn.ReLU()
        self.batch_norm = nn.BatchNorm2d(num_nodes)
        # attentions is an nn.ModuleList above, so net.cuda()/.to(device) moves the
        # heads automatically. The old manual `att.cuda()` loop reassigned
        # self.attentions to a plain list, which un-registered the heads (hiding them
        # from .eval(), .parameters() and state_dict() -> dropout stayed on at
        # inference, and the heads were never trained or saved).
